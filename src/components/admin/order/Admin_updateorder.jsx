...
function Admin_updateorder() {
  ...
  const [customerInfo, setCustomerInfo] = useState({
    ...
    cust_number: '',
    cust_addresses: [],
    selected_address: 0,  // Default to first address index
    pincode: '',
    order_date: '',
  });
  const [newAddress, setNewAddress] = useState('');
    ...
    const fetchOrderDetails = async () => {
      try {
        ...
        setCustomerInfo({
          ...
          cust_number: order.cust_number || '',
          cust_addresses: order.cust_address || [],
          selected_address: order.selected_address || 0,  // Default to the first address
          pincode: order.pincode || '',
          order_date: order.order_date ? order.order_date.split('T')[0] : '',
        });
      } catch (error) {
...
const handleSelectAddress = (index) => {
  setCustomerInfo((prevState) => ({
    ...prevState,
    selected_address: index + 1, // Convert to 1-based index
  }));
};
  ...
  const handleSubmit = async (e) => {
    ...
    const updatedOrderData = {
      ...customerInfo, // Keep other fields unchanged
      cust_address: customerInfo.cust_address, // Addresses
      selected_address: customerInfo.selected_address, // 1-based selected address
    };
              ...
              {customerInfo.cust_addresses.map((address, index) => (
                <div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                  <input
                    ...
                    checked={customerInfo.selected_address === index + 1} // Check if selected
                    onChange={() => handleSelectAddress(index)}
                  />
